...
def main():
    ...
    # 🚀🚀🚀 进阶核心：构建非对称权重 (Asymmetric Sample Weights)
    sample_weights = np.ones(len(y_train_xgb))
    
    # 基础偏置：给目标域（微调集） 10 倍权重，让模型更关注新工况
    sample_weights[len(X_src):] = 10.0 
    
    # 进阶偏置：对于所有 SOH < 0.85 的样本（生命中后期），权重再翻 1.5 倍
    # 这将强制模型在预测那些退化深坑时，不敢轻易给出偏高的预测值
    low_soh_mask = (y_train_xgb < 0.85)
    sample_weights[low_soh_mask] *= 1.5

    xgb_model = xgb.XGBRegressor(
        n_estimators=180,       
        learning_rate=0.03,     
        max_depth=6,            # 深度 6：具有足够的非线性拟合能力
        subsample=0.8,
