...
def main():
    ...
    # 将 nn.MSELoss() 替换为 nn.L1Loss() (平均绝对误差)
    criterion = nn.L1Loss()
    ...
    xgb_model = xgb.XGBRegressor(
        n_estimators=180,       # 🚀 既然用 L1 (or Custom Asymmetric L1)，需要更多的树来迭代拟合
        learning_rate=0.03,     # 维持细腻的学习率
        # 🚀 修改：加深树深。从 4 改为 6 甚至 7。
        # 既然我们用的是 19D 物理特征（不是那个会导致过拟合的几百维深度特征），
        # 我们可以允许树深增加，以拟合电池退化的非线性深坑。
        max_depth=6,            
        subsample=0.8,
        colsample_bytree=0.8,   
        # 🚀 可选：结合第一步的技术派方案，这里用自定义损失。
        # 如果还没写好自定义损失，暂时继续用 reg:absoluteerror。
        objective='reg:absoluteerror', 
        # 🚀 修改：调低正则化参数。昨天加的 1.5/0.5 此时可能太强了。
        reg_lambda=0.5,         # 🚀 新增：调小 L2 正则，允许模型拟合更陡峭的变化
        reg_alpha=0.1,          # 🚀 新增：调小 L1 正则，保留更多对非线性拟合有益的特征
