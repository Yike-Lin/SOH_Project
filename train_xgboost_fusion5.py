...
def get_args():
    ...
    p.add_argument('--save_folder', default='results_XGBoost_Fusion_CNN2')
    return p.parse_args()
...
def main():
    ...
    xgb_model = xgb.XGBRegressor(
        n_estimators=250,       # 🚀 增加树的数量，充分利用 43 维特征
        learning_rate=0.05,     # 🚀 稍微提速
        max_depth=7,            # 🚀 加深一层，捕获更复杂的物理边界非线性关系
        min_child_weight=3,     # 🚀 新增：叶子节点最小权重，防止过拟合极端离群点
        gamma=0.1,              # 🚀 新增：节点分裂的最小损失下降值，自动剪枝
        subsample=0.85,
        colsample_bytree=0.85,   
        objective='reg:absoluteerror', 
        reg_lambda=1.0,         # 🚀 加强 L2 正则
        reg_alpha=0.1,          
    ...
    if str(args.test_batch) == '5':
        print("🔧 Applying light Savitzky-Golay smoothing for Batch 5...")
        for bid in unique_bids:
            mask = (eval_batt_ids == bid)
            y_p_batt = y_pred_xgb[mask]
            
            # 🚀 加入 mode='nearest' 解决末端上翘发散问题！
            if len(y_p_batt) > 15:
                y_p_batt = savgol_filter(y_p_batt, window_length=15, polyorder=3, mode='nearest')
            elif len(y_p_batt) > 7:
                y_p_batt = savgol_filter(y_p_batt, window_length=7, polyorder=2, mode='nearest')
            
            # 🚀 还可以加一个强硬的物理限制：当前的预测值绝不应该比初始 SOH 高太多（例如限制最大值为1.0）
            y_p_batt = np.clip(y_p_batt, a_min=0, a_max=1.0)
